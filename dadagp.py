...
def parse_decode_metadata(tokens):
    """Return title and artist metadata for decoded token streams.

    Extended token files may start with bracketed metadata before the original
    artist token. When [ARTIST:] is absent, preserve the original fallback by
    using the first non-bracket token rather than tokens[0].
    """
    metadata_title = None
    ...
    return {
        "title": metadata_title if metadata_title else "untitled",
        "artist": metadata_artist if metadata_artist else artist_token,
    }
...
def dadagp_decode(input_file, output_file):
    ...
    decoded_metadata = parse_decode_metadata(tokens)
    song.title  = decoded_metadata["title"]
    song.artist = decoded_metadata["artist"]
